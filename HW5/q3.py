...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = torch.sum((D.forward(real_images) - 1) ** 2) / len(real_images) / 2
            # 2. Sample noise
            # noise = ...
            ...
            # D_fake_loss = ...
            D_fake_loss = torch.sum(D.forward(fake_images) ** 2) / len(real_images) / 2
            ...
            # G_loss = ...
            G_loss = torch.sum((D.forward(fake_images) - 1) ** 2) / len(real_images)
            G_loss.backward()
            g_optimizer.step()
